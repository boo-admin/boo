...
const (
	...
	IPv4RangeV1 = 1000 // x.x.x.a-x.x.x.b
	IPv4RangeV2 = 1001 // x.x.x.x-y.y.y.y
)
...
func ValidIPAddr(cktypes ...int) IPAddr {
	...
	for _, cktype := range cktypes {
		...
		if cktype != IPAny &&
			cktype != IPv4 &&
			cktype != IPv6 &&
			cktype != IPv4MappedIPv6 &&
			cktype != IPv4CIDR &&
			cktype != IPv6CIDR &&
			cktype != IPv4MappedIPv6CIDR &&
			cktype != IPv4RangeV1 &&
			cktype != IPv4RangeV2 {
...
func (i IPAddr) IsSatisfied(obj interface{}) bool {
	...
	if str, ok := obj.(string); ok {
		...
		for _, ck := range i.Vaildtypes {
			...
			case IPv4MappedIPv6CIDR:
				if ret == IPv4MappedIPv6CIDR {
					ip, _, err := net.ParseCIDR(str)
					if err == nil && ip.To4() != nil {
						return true
					}
			...
			case IPv4RangeV1:
				if ss := strings.Split(str, "-"); len(ss) == 2 {
					start := net.ParseIP(ss[0])
